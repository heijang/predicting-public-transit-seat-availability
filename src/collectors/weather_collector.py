...
DEFAULT_OUTPUT_DIR = Path(__file__).parent.parent.parent / 'data' / 'realtime' / 'weather'
...
def fetch_weather_data(lat: float = BERLIN_LAT, lon: float = BERLIN_LON,
                       timeout: int = 30, forecast_hours: int = 1) -> dict:
    ...
    Args:
        ...
        forecast_hours: Hours ahead to forecast (default: 1)
    """
    params = {
        'latitude': lat,
        'longitude': lon,
    ...
    data = response.json()
    data['_forecast_hours'] = forecast_hours
    return data
...
def parse_weather_response(data: dict) -> dict:
    ...
    Extracts forecast data for N hours ahead (default: 1 hour).
    """
    hourly = data.get('hourly', {})
    daily = data.get('daily', {})
    forecast_hours = data.get('_forecast_hours', 1)

    fetch_time = datetime.now()

    # Find the index for the target forecast hour
    # Hourly times are like "2026-01-20T00:00", "2026-01-20T01:00", etc.
    times = hourly.get('time', [])

    # Find current hour index, then add forecast_hours
    current_hour = fetch_time.hour
    target_idx = current_hour + forecast_hours
    target_idx = min(target_idx, len(times) - 1) if times else 0

    def get_hourly_value(key, idx=target_idx):
    ...
    record = {
        ...
        # Forecast weather (1 hour ahead)
        'forecast_time': times[target_idx] if target_idx < len(times) else None,
        'forecast_hours_ahead': forecast_hours,
        'temperature': get_hourly_value('temperature_2m'),
        'apparent_temperature': get_hourly_value('apparent_temperature'),
        'humidity': get_hourly_value('relative_humidity_2m'),
        'precipitation': get_hourly_value('precipitation'),
        'rain': get_hourly_value('rain'),
        'snowfall': get_hourly_value('snowfall'),
        'weather_code': get_hourly_value('weather_code'),
        'cloud_cover': get_hourly_value('cloud_cover'),
        'wind_speed': get_hourly_value('wind_speed_10m'),
        'wind_direction': get_hourly_value('wind_direction_10m'),
...
def collect_weather_data(output_dir: Path = None) -> dict:
    ...
    print(f"Forecast time: {record['forecast_time']} (+{record['forecast_hours_ahead']}h)")
